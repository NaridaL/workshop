...
vec4 openSimplex2SDerivativesPart(vec3 X) {
  ...
  vec3 v2 =
    b +
    vec3(1, 0, 0) +
    vec3(-1, 1, 1) * floor(dot(i4, vec4(-0.25, 0.25, 0.25, 0.35)));
  vec3 v3 =
    b +
    vec3(0, 1, 0) +
    vec3(1, -1, 1) * floor(dot(i4, vec4(0.25, -0.25, 0.25, 0.35)));
  ...
  vec3 d4 = X - v4;
  vec4 a = max(
    0.75 - vec4(dot(d1, d1), dot(d2, d2), dot(d3, d3), dot(d4, d4)),
    0.0
  );
  vec4 aa = a * a;
  vec4 aaaa = aa * aa;
...
vec4 openSimplex2SDerivatives_Conventional(vec3 X) {
  ...
  vec4 result =
    openSimplex2SDerivativesPart(X) + openSimplex2SDerivativesPart(X + 144.5);
